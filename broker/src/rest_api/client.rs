...
use crate::{
    app::YedMQApp, protobuf::GetSessionInfoRequest, raft::session_actor_map::session_actor_map_raft_actor::{GetClientListWithPagination, SessionActorMapRaftError}, session::{
...
pub async fn client_list(
    ...
    match app.service_registry.session_map_raft.send(
        GetClientListWithPagination {
            tenant_id: tenant_id.clone(),
            offset: offset_param as usize,
            limit: limit_param as usize,
        }
    ).await.expect("Session actor map actor not ready") {
        Ok(response) => {
            ...
            for (client_id, node_id) in response.client_list {
                ...
                if let Some(mut cluster_rpc_client) = cluster_rpc_client_option {
                    let get_session_info_response = cluster_rpc_client.get_session_info(
                    ...
                    if let Ok(response) = get_session_info_response {
                        ...
                        let activaty_state = match crate::protobuf::ActivityState::try_from(session_info.session_state).unwrap_or(crate::protobuf::ActivityState::Inactive) {
                        ...
                        error!("get session info rpc error: {:?}", get_session_info_response.err());
                    }
                }
            }
            ...
            let result = PaginationListResult { meta, data: clients };
