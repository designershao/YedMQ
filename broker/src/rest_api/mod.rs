...
use crate::{plugin_manager, session::session_manager::{self, SessionManagerError, SessionState}, settings::Settings, topic};
...
async fn topic_list(
    ...
    let topic_list_result = app_state.topic_manager.read().await.get_topic_list_with_pagination(tenant_id, offset_param, limit_param);

    if let Err(err) = topic_list_result {
        error!("get topic list error: {}", err);
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(PaginationListResult {
            meta: PaginationMeta {
                offset: offset_param,
                limit: limit_param,
                total: 0
