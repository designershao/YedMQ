...
impl Inflight {
    ...
    pub fn register_with_rx_packet(&mut self, packet_identifier: u16, qos: u8, packet_key: String) -> Option<String> {
    ...
    pub fn register_with_tx_packet(&mut self, packet_identifier: u16, qos: u8, packet_key: String) -> Result<Option<String>, InflightError> {
    ...
    pub fn get_all_expired_packet_keys_and_refresh_expired_time(&mut self) -> Vec<(u16, String)> {
        ...
        //let mut inner = self.inner.write().await;
        for item in self.inner.values_mut() {
            if item.last_modified + self.expired_duration.as_secs() < std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() {
                if let Some(packet_key) = &item.packet_key {
                    result_vec.push((item.packet_identifier, packet_key.clone()));
                }
                item.last_modified = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs();
