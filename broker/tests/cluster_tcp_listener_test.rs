...
async fn test_publish_subscribe_cross_node(qos: QoS) {
    ...
    let sub_addr: SocketAddr = sub_node.listener.tcp.external.as_str().parse().unwrap();

    let topic = format!("test/cluster/{:?}", qos);
    ...
    // Wait slightly for sub to be ready and cluster to propagate subscription?
    // In a eventually consistent or Raft cluster, subscription info might need time?
    // Or just wait for SubAck.
...
async fn test_publish_subscribe_qos0() {
    test_publish_subscribe_cross_node(QoS::AtMostOnce).await;
}

#[actix::test]
async fn test_publish_subscribe_qos1() {
    test_publish_subscribe_cross_node(QoS::AtLeastOnce).await;
}

#[actix::test]
async fn test_publish_subscribe_qos2() {
    test_publish_subscribe_cross_node(QoS::ExactlyOnce).await;
}

#[actix::test]
async fn test_retained_message_cross_node() {
    let context = setup_cluster().await;
    ...
    let topic = "test/cluster/retained";
...
async fn test_last_will_message_cross_node() {
    ...
    let will_topic = "test/cluster/will";
    let will_message = b"cluster unclean disconnect";
...
async fn test_persistent_session_cross_node() {
    ...
    // Node 2 (Sub connects here, persistent)
    let sub_node = &context.nodes[1];
    let sub_addr: SocketAddr = sub_node.listener.tcp.external.as_str().parse().unwrap();

    let topic = "test/cluster/persistent";
