...
use yedmq::settings::{Settings, Node};
use rand::Rng;
    ...
    ASYNC_SETUP.get_or_init(|| async {
        ...
        */
        ...
        for (i, &id) in node_ids.iter().enumerate() {
            ...
            let absolute_certs_path = fs::canonicalize(&certs_path).expect("Failed to canonicalize certs path");
             ...
             let settings = Settings {
                ...
                plugin: yedmq::settings::Plugin {
                    ...
                    local_socket_path: format!("{}/yedmq_plugin_host_{}.sock", temp_dir.path().to_str().unwrap(), id),
