...
fn raw_tls_client_config() -> Arc<direct_rustls::ClientConfig> {
    let crate_root_path = env!("CARGO_MANIFEST_DIR");
    let certs_path = PathBuf::from(crate_root_path).join("tests").join("certs");
    let ca_file_path = certs_path.join("ca.crt");
...
async fn connect_mqtt5_tls_client(
...
pub async fn test_tls_listener_connect() {
    ...
    let broker_addr: SocketAddr = settings.listener.tcp_tls.external.as_str().parse().unwrap();

    let mut options = MqttOptions::new("test_cluster_tls_connect", "localhost", broker_addr.port());
...
#[actix::test]
pub async fn test_tls_listener_mqtt5_publish_subscribe_cross_node_smoke() {
    let context = setup_cluster().await;
    ...
    let topic = format!("test/cluster/mqtt5/tls/{suffix}");
    let payload = b"hello cluster mqtt5 tls";

    let mut subscriber = connect_mqtt5_tls_client(sub_addr, "cluster-mqtt5-tls-sub").await;
    common::mqtt5::write_packet_to_writer(
        &mut subscriber,
    ...
    let mut publisher = connect_mqtt5_tls_client(pub_addr, "cluster-mqtt5-tls-pub").await;
    common::mqtt5::write_packet_to_writer(
        &mut publisher,
    ...
    let publish = common::mqtt5::read_packet_from_reader(&mut subscriber).await;
...
async fn test_tls_publish_subscribe_cross_node(qos: QoS) {
    let context = setup_cluster().await;

    // Publisher -> Node 1
    let pub_node = &context.nodes[0];
    let pub_addr: SocketAddr = pub_node.listener.tcp_tls.external.as_str().parse().unwrap();

    // Subscriber -> Node 2
    let sub_node = &context.nodes[1];
    let sub_addr: SocketAddr = sub_node.listener.tcp_tls.external.as_str().parse().unwrap();

    let topic = format!("test/cluster/tls/{:?}", qos);
    let payload = format!("hello cluster tls {:?}", qos).into_bytes();

    // Subscriber Setup
    let mut sub_opts = MqttOptions::new(format!("sub-tls-{:?}", qos), "localhost", sub_addr.port());
    sub_opts.set_keep_alive(Duration::from_secs(5));
    sub_opts.set_transport(configure_tls());
    ...
    // Publisher Setup
    let mut pub_opts = MqttOptions::new(format!("pub-tls-{:?}", qos), "localhost", pub_addr.port());
    pub_opts.set_keep_alive(Duration::from_secs(5));
    pub_opts.set_transport(configure_tls());
...
async fn test_tls_publish_subscribe_qos0() {
    test_tls_publish_subscribe_cross_node(QoS::AtMostOnce).await;
}

#[actix::test]
async fn test_tls_publish_subscribe_qos1() {
    test_tls_publish_subscribe_cross_node(QoS::AtLeastOnce).await;
}

#[actix::test]
async fn test_tls_publish_subscribe_qos2() {
    test_tls_publish_subscribe_cross_node(QoS::ExactlyOnce).await;
}

#[actix::test]
async fn test_tls_retained_message_cross_node() {
    let context = setup_cluster().await;
    let pub_node = &context.nodes[0];
    let sub_node = &context.nodes[1];
    ...
    let topic = "test/cluster/tls/retained";
    let payload = b"cluster tls retained message";

    // Client 1 publishes retained to Node 1
    let mut mqtt_options1 = MqttOptions::new("retained-tls-pub", "localhost", pub_addr.port());
    ...
    let mut mqtt_options2 = MqttOptions::new("retained-tls-sub", "localhost", sub_addr.port());
...
#[actix::test]
async fn test_tls_last_will_message_cross_node() {
    let context = setup_cluster().await;

    let node1 = &context.nodes[0];
    let addr1: SocketAddr = node1.listener.tcp_tls.external.as_str().parse().unwrap();

    let node2 = &context.nodes[1];
    let addr2: SocketAddr = node2.listener.tcp_tls.external.as_str().parse().unwrap();

    let will_topic = "test/cluster/tls/will";
    let will_message = b"cluster tls unclean disconnect";
    let last_will = LastWill::new(will_topic, will_message, QoS::AtLeastOnce, false);

    // Subscriber
    let mut sub_opts = MqttOptions::new("will-tls-sub", "localhost", addr2.port());
    sub_opts.set_keep_alive(Duration::from_secs(5));
    sub_opts.set_transport(configure_tls());
    let (sub_client, mut sub_eventloop) = AsyncClient::new(sub_opts, 10);
    ...
    // Will Client
    let mut will_opts = MqttOptions::new("will-tls-client", "localhost", addr1.port());
    will_opts
        .set_keep_alive(Duration::from_secs(2))
        .set_last_will(last_will);
    will_opts.set_transport(configure_tls());
...
async fn test_persistent_session_cross_node() {
    ...
    let topic = "test/cluster/tls/persistent";
    let payload = b"cluster tls persistent message";

    // Sub Client (Persistent)
    let mut sub_opts = MqttOptions::new("cluster-persistent-tls-sub", "localhost", sub_addr.port());
    sub_opts.set_keep_alive(Duration::from_secs(5));
    sub_opts.set_transport(configure_tls());
    ...
    let mut sub_opts2 =
        MqttOptions::new("cluster-persistent-tls-sub", "localhost", sub_addr.port());
    sub_opts2.set_keep_alive(Duration::from_secs(5));
    sub_opts2.set_transport(configure_tls());
