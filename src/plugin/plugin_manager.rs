...
use tokio::{runtime::Builder, sync::mpsc::error::SendError};
...
impl PluginManager {
    ...
    // Load new plugin from the path
    pub async fn load(&mut self, path: &String) -> anyhow::Result<()> {
        ...
        std::thread::spawn(move || {
            ...
            local_set.spawn_local(async move {
                ...
                if let Err(err) = init_tx.send(PluginManagerMessage::PluginLoadAllFinished).await {
                    warn!("load plugin error: {}", err);
        ...
        loop {
            ...
            match msg {
                PluginManagerMessage::PluginStartSucceed((name, tx)) => {
                    if let Some(tx_old) = self.plugin_table.get(&name.clone()) {
                        tx_old.send(PluginMessage::Quit).await.unwrap(); // notify stop the old plugin instance
                    }
                    self.plugin_table.insert(name, tx);
                },
                PluginManagerMessage::PluginLoadAllFinished => {
                    break;
                },
...
mod tests {
    ...
    pub async fn test_plugin_manager_load() {
        ...
        let mut plugin_manager = PluginManager::new(&plugin_path.to_str().unwrap().to_string()).await.unwrap();
