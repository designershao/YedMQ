...
impl PubRecPacket {
   pub fn new(packet_identifier: u16) -> PubRecPacket {
...
mod tests{
    ...
    fn test_to_bytes() {
        ...
        let puback_packet = PubRecPacket {
            fix_header,
            variable_header
        };

        assert_eq!(puback_packet.to_bytes().as_bytes(), &[0x50, 0x02, 0x00, 0x0A]);
